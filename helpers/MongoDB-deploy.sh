...
until [ $(kubectl get om -n $OPS_MANAGER_NAMESPACE -o=jsonpath='{.items[0].status.applicationDatabase.phase}') == Running ];
do
sleep 10s
done;
...
if [[ $(mongocli iam organizations ls --name $OPS_MANAGER_ORG_NAME -o go-template="{{ len .Results }}") > 0 ]] 
then
...
mongocli iam organizations apiKeys create --orgId $(mongocli iam organizations describe $OPS_MANAGER_ORG_ID -o go-template="{{ .ID }}") --desc "My API key" --role ORG_OWNER -o go-template='kubectl -n '${MONGODB_NAMESPACE}' create secret generic opsmanager-org-access-key  --from-literal="user={{.PublicKey}}"  --from-literal="publicApiKey={{ .PrivateKey }}" ' | bash 
# create config map
kubectl create configmap opsmanager-configmap --from-literal=projectName=$MONGODB_NAME --from-literal=baseUrl="http://$(kubectl -n $OPS_MANAGER_NAMESPACE get svc $OPS_MANAGER_HELM_RELEASE_NAME-ops-manager-svc-ext -o=jsonpath='{.status.loadBalancer.ingress[0].hostname}'):8080" --from-literal=orgId=$OPS_MANAGER_ORG_ID
